...
2. Přejděte do sekce **Source Control**

    >(ikona větvení v levém panelu nebo zkratka `Ctrl + Shift + G`).

3. Klikněte na modré tlačítko `Publish to GitHub`.

    >Pokud tlačítko nevidíte, stiskněte `F1`(nebo `Ctrl + Shift + P`) a napište příkaz `Publish to GitHub`.
